...
object SingleStatementTable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val RDFDF = spark.read.format("com.databricks.spark.avro").load(s"$path/SingleStmtTable").toDF()
    RDFDF.createOrReplaceTempView("SingleStmtTable")


    val fos = new FileOutputStream(new File(s"/home/cloudera/Downloads/Results/avro/ST/$ds.txt"),true)
