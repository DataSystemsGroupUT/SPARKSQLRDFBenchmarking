...
object VerticalTablesPartition {
  def main(args: Array[String]): Unit = {
    ...
    val vpText = spark.read.option("header", true).csv(path + "VHDFS/CSV/" + "sorg_text.csv").toDF()
    ...
    val vpdescription = spark.read.option("header", true).csv(path + "VHDFS/CSV/" + "sorg_description.csv").toDF()
    ...
    if (partitionType == "subject") {
      ...
      vpText.repartition(84, $"Subject").write.option("header", true).format("csv").mode(SaveMode.Overwrite).save(path + "Subject/CSV/" + "sorg_text.csv")
      ...
      vpdescription.repartition(84, $"Subject").write.option("header", true).format("csv").mode(SaveMode.Overwrite).save(path + "Subject/CSV/" + "sorg_description.csv")
      ...
      vpText.repartition(84).write.option("header", true).format("csv").mode(SaveMode.Overwrite).save(path + "Horizontal/CSV/" + "sorg_text.csv")
      ...
      vpdescription.repartition(84).write.option("header", true).format("csv").mode(SaveMode.Overwrite).save(path + "Horizontal/CSV/" + "sorg_description.csv")
