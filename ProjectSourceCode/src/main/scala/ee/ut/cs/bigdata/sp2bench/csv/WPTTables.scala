...
object WPTTables {
  def main(args: Array[String]): Unit = {
    ...
    val wpt1 = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(s"$path/WPTType$partitionType.csv").toDF()
              .groupBy("Subject")
              .agg(collect_list("type") as "type")

    val wpt2 = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(s"$path/WPTName$partitionType.csv").toDF()
              .groupBy("Subject")
              .agg(collect_list("name") as "name")

    val wpt3 = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(s"$path/WPTTitle$partitionType.csv").toDF()
              .groupBy("Subject")
