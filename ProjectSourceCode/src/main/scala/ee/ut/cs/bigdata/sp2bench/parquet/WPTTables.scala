...
object WPTTables {
  def main(args: Array[String]): Unit = {
    println("Parquet WPT Partitioned")
    ...
    val wpt1 = spark.read.format("parquet").load(s"$path/WPTType$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("type") as "type")

    val wpt2 = spark.read.format("parquet").load(s"$path/WPTName$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("name") as "name")

    val wpt3 = spark.read.format("parquet").load(s"$path/WPTTitle$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("title") as "title")

    val wpt4 = spark.read.format("parquet").load(s"$path/WPTIssued$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("issued") as "issued")

    val wpt5 = spark.read.format("parquet").load(s"$path/WPTCreator$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("creator") as "creator")

    val wpt6 = spark.read.format("parquet").load(s"$path/WPTHomepage$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("homepage") as "homepage")

    val wpt7 = spark.read.format("parquet").load(s"$path/WPTSeeAlso$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("seeAlso") as "seeAlso")

    val wpt8 = spark.read.format("parquet").load(s"$path/WPTBooktitle$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("booktitle") as "booktitle")

    val wpt9 = spark.read.format("parquet").load(s"$path/WPTPartOf$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("partOf") as "partOf")

    val wpt10 = spark.read.format("parquet").load(s"$path/WPTAbstract$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("abstract") as "abstract")

    val wpt11 = spark.read.format("parquet").load(s"$path/WPTPages$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("pages") as "pages")

    val wpt12 = spark.read.format("parquet").load(s"$path/WPTJournal$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("journal") as "journal")

    val wpt13 = spark.read.format("parquet").load(s"$path/WPTSeries$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("series") as "series")

    val wpt14 = spark.read.format("parquet").load(s"$path/WPTNumber$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("number") as "number")

    val wpt15 = spark.read.format("parquet").load(s"$path/WPTNote$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("note") as "note")

    val wpt16 = spark.read.format("parquet").load(s"$path/WPTVolume$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("volume") as "volume")

    val wpt17 = spark.read.format("parquet").load(s"$path/WPTSubClassOf$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("subClassOf") as "subClassOf")

    val wpt18 = spark.read.format("parquet").load(s"$path/WPTMonth$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("month") as "month")

    val wpt19 = spark.read.format("parquet").load(s"$path/WPTIsbn$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("isbn") as "isbn")

    val wpt20 = spark.read.format("parquet").load(s"$path/WPTEditor$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("editor") as "editor")

    val wpt21 = spark.read.format("parquet").load(s"$path/WPTPublisher$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("publisher") as "publisher")

    val wpt22 = spark.read.format("parquet").load(s"$path/WPTReferences$partitionType.parquet").toDF()
              .groupBy("Subject")
              .agg(collect_list("references") as "references")

    val wpt23 = spark.read.format("parquet").load(s"$path/WPTCdrom$partitionType.parquet").toDF()
              .groupBy("Subject")
    ...
    {
    val RDFDFWPT = spark.read.format("parquet").load(s"$path/WPT$partitionType.parquet").toDF()
    RDFDFWPT.createOrReplaceTempView("WPT")
    }

    //create file to write the query run time results    
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs2/$ds/parquet/WPT/$ds$partitionType.txt"),true)
