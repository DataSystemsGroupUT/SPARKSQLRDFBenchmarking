...
object WPTTablesPartition {
  def main(args: Array[String]): Unit = {
    ...
    if (partitionType == "subject")
    {
     ...
     RDFDFWPT.repartition(84, $"Subject").write.option("header", "true").format("csv").mode(SaveMode.Overwrite).save(s"$path/WPTSubject.csv")  
    ...
    else if (partitionType == "horizontal")
    {
     ...
     RDFDFWPT.repartition(84).write.option("header", "true").format("csv").mode(SaveMode.Overwrite).save(s"$path/WPTHorizontal.csv")
