...
object VerticalTablesQP {
  def main(args: Array[String]): Unit = {
    ...
    FileSystem.get(sc.hadoopConfiguration).listStatus(new Path(s"$path/$partitionType/CSV")).foreach {
      x =>
        val vpTable = spark.read.option("header", true).csv(x.getPath().toString)
        vpTable.createOrReplaceTempView(x.getPath().getName().substring(0, x.getPath().getName().lastIndexOf('.')))
    }


    //create file to write the query run time results
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs/$ds/csv/VP/$partitionType.txt"), true)
