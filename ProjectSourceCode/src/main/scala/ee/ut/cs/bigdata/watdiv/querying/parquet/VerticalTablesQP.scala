...
object VerticalTablesQP {
  def main(args: Array[String]): Unit = {
    ...
    FileSystem.get(sc.hadoopConfiguration).listStatus(new Path(s"$path/$partitionType/Parquet")).foreach {
      x =>
        val vpTable = spark.read.format("parquet").load(x.getPath().toString)
        vpTable.createOrReplaceTempView(x.getPath().getName().substring(0, x.getPath().getName().lastIndexOf('.')))
    }


    //create file to write the query run time results
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs/$ds/parquet/VP/$partitionType.txt"), true)
