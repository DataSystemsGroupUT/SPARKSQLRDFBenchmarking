...
class ExtVPQueries {
  ...
  val F1 =
    """
      ...
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/Topic8') tab0
      |JOIN  (SELECT subject AS v0 , object AS v2
  ...
  val F2 =
    """
      ...
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/SubGenre117') tab7
      |JOIN    (SELECT object AS v1 , subject AS v0
      |FROM SS_homepage_caption) tab0
      |ON(tab7.v0=tab0.v0)
      |JOIN    (SELECT subject AS v0 , object AS v4
      |FROM SS_caption_homepage) tab3
      |ON(tab0.v0=tab3.v0)
      |JOIN    (SELECT subject AS v0 , object AS v5
      |FROM SS_description_caption) tab4
      |ON(tab3.v0=tab4.v0)
      |JOIN    (SELECT subject AS v0 , object AS v2
      |FROM SS_title_caption) tab1
      |ON(tab4.v0=tab1.v0)
      |JOIN    (SELECT subject AS v0 , object AS v3
      |FROM SS_type_caption) tab2
      |ON(tab1.v0=tab2.v0)
      |JOIN    (SELECT subject AS v1 , object AS v6
      |FROM SO_url_homepage) tab5
      |ON(tab0.v1=tab5.v1)
      |JOIN    (SELECT subject AS v1 , object AS v7
      |FROM SO_hits_homepage) tab6
      |ON(tab5.v1=tab6.v1)
      |""".stripMargin
  ...
  val F3 =
    """
      ...
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/SubGenre111') tab2
      |JOIN    (SELECT object AS v1 , subject AS v0
      |FROM SS_contentRating_contentSize) tab0
      |ON(tab2.v0=tab0.v0)
      |JOIN    (SELECT subject AS v0 , object AS v2
      |FROM SS_contentSize_contentRating) tab1
      |ON(tab0.v0=tab1.v0)
      |JOIN    (SELECT object AS v0 , subject AS v5
      |FROM OS_purchaseFor_contentSize) tab5
      |ON(tab1.v0=tab5.v0)
      |JOIN    (SELECT object AS v5 , subject AS v4
      |FROM OS_makesPurchase_purchaseDate) tab3
      |ON(tab5.v5=tab3.v5)
      |JOIN    (SELECT subject AS v5 , object AS v6
      |FROM VP_purchaseDate) tab4
      |ON(tab3.v5=tab4.v5)
      |""".stripMargin
  ...
  val F4 =
    """
      ...
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/Language0') tab7
      |JOIN    (SELECT object AS v1 , subject AS v0
      |FROM SS_homepage_contentSize) tab0
      |ON(tab7.v1=tab0.v1)
      |JOIN    (SELECT subject AS v0
      |FROM SS_tag_contentSize
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/Topic122') tab2
      |ON(tab0.v0=tab2.v0)
      |JOIN    (SELECT subject AS v0 , object AS v8
      |FROM SS_contentSize_homepage) tab4
      |ON(tab2.v0=tab4.v0)
      |JOIN    (SELECT subject AS v0 , object AS v4
      |FROM SS_description_contentSize) tab3
      |ON(tab4.v0=tab3.v0)
      ...
      |FROM OS_includes_contentSize) tab1 ON(tab3.v0=tab1.v0)
      |JOIN    (SELECT object AS v0 , subject AS v7
      |FROM OS_likes_contentSize) tab8
      |ON(tab1.v0=tab8.v0)
      |""".stripMargin
  ...
  val F5 =
    """
      ...
      |WHERE subject = 'http://db.uwaterloo.ca/~galuc/wsdbm/Retailer9885') tab1
      |JOIN    (SELECT subject AS v0 , object AS v4
  ...
  val L2 =
    """
      ...
      |WHERE subject = 'http://db.uwaterloo.ca/~galuc/wsdbm/City70' ) tab0
  ...
  val L3 =
    """
      ...
      |FROM    (SELECT subject AS v0
      |FROM SS_subscribes_likes
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/Website43164') tab0
      |JOIN    (SELECT object AS v1 , subject AS v0
  ...
  val L4 =
    """
      ...
      |FROM    (SELECT subject AS v0
      |FROM SS_tag_caption
      |WHERE object = 'http://db.uwaterloo.ca/~galuc/wsdbm/Topic142') tab0
      |JOIN    (SELECT subject AS v0 , object AS v2
