...
import ee.ut.cs.bigdata.watdiv.querying.queries.STQueries
...
object SingleStatementTableQP {
  def main(args: Array[String]): Unit = {
    ...
    val ST_DF = spark.read.format("parquet").load(s"$path/$partitionType/Parquet/ST$ds.parquet").toDF()

    ST_DF.createOrReplaceTempView("Triples")

    //create file to write the query run time results
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs/$ds/parquet/ST/$partitionType.txt"), true)
