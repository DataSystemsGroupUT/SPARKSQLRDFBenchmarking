...
object ExtVerticalTablesQP {
  def main(args: Array[String]): Unit = {
    ...
    FileSystem.get(sc.hadoopConfiguration).listStatus(new Path(s"$path/$partitionType/Parquet")).foreach {
      x =>
        val extVPTable = spark.read.format("parquet").load(x.getPath().toString)
        extVPTable.createOrReplaceTempView(x.getPath().getName().substring(0, x.getPath().getName().lastIndexOf('.')))
    }


    //create file to write the query run time results
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs2/watdiv/$ds/parquet/ExtVP/$partitionType$ds.txt"), true)
