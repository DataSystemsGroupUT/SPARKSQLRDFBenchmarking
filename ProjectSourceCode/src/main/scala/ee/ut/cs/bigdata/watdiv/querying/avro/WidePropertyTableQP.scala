...
object WidePropertyTableQP {
  def main(args: Array[String]): Unit = {
    ...
    println("Starting Querying WPT Partitioned Avro!")
    ...
    val spark = SparkSession
      .builder()
      .appName("RDFBench Avro WPT Partitioned")
      .getOrCreate()
    ...
    val WPT_DF = spark.read.format("avro").load(s"$path/$partitionType/Avro/WPT$ds.avro").toDF()

    WPT_DF.createOrReplaceTempView("WPT")

    //create file to write the query run time results
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs/$ds/avro/WPT/$partitionType.txt"), true)
