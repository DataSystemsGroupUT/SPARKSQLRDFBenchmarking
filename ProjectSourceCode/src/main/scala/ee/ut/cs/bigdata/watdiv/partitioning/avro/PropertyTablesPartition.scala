...
object PropertyTablesPartition {
  def main(args: Array[String]): Unit = {
    ...
    val Retailer_DF = spark.read.format("avro").load(path + "Avro/Retailer.avro")
    val Website_DF = spark.read.format("avro").load(path + "Avro/Website.avro")
    val Product_DF = spark.read.format("avro").load(path + "Avro/Product.avro")
    val User_DF = spark.read.format("avro").load(path + "Avro/User.avro")
    val Offer_DF = spark.read.format("avro").load(path + "Avro/Offer.avro")
    val City_DF = spark.read.format("avro").load(path + "Avro/City.avro")
    val Review_DF = spark.read.format("avro").load(path + "Avro/Review.avro")
    val Role_DF = spark.read.format("avro").load(path + "Avro/Role.avro")
    val Genre_DF = spark.read.format("avro").load(path + "Avro/Genre.avro")
    val Trailer_DF = spark.read.format("avro").load(path + "Avro/Trailer.avro")
    val Purchase_DF = spark.read.format("avro").load(path + "Avro/Purchase.avro")
    val Language_DF = spark.read.format("avro").load(path + "Avro/Language.avro")
    val Likes_DF = spark.read.format("avro").load(path + "Avro/Likes.avro")
    val Subscribes_DF = spark.read.format("avro").load(path + "Avro/Subscribes.avro")
    val EligibilityRegion_DF = spark.read.format("avro").load(path + "Avro/EligibilityRegion.avro")
    val HasGenre_DF = spark.read.format("avro").load(path + "Avro/HasGenre.avro")
    val MakesPurchase_DF = spark.read.format("avro").load(path + "Avro/MakesPurchase.avro")
    val Tag_DF = spark.read.format("avro").load(path + "Avro/Tag.avro")
    val Includes_DF = spark.read.format("avro").load(path + "Avro/Includes.avro")
    val Offers_DF = spark.read.format("avro").load(path + "Avro/Offers.avro")
    val HasReview_DF = spark.read.format("avro").load(path + "Avro/HasReview.avro")
    val FriendOf_DF = spark.read.format("avro").load(path + "Avro/FriendOf.avro")
    val Actor_DF = spark.read.format("avro").load(path + "Avro/Actor.avro")
    val PurchaseFor_DF = spark.read.format("avro").load(path + "Avro/PurchaseFor.avro")
    ...
    if (partitionType.toLowerCase == "subject") {
      ...
      retailerAggregateRating.repartition(84).write.option("header", "true").format("avro").mode(SaveMode.Overwrite).save(path + "Predicate/Avro/retailerAggregateRating.avro")
          ...
          println("Original count: " + Retailer_DF.count())
          Retailer_DF.printSchema()

          println(retailer_join9.toDF().count())
          retailer_join9.toDF().printSchema()
          ...
          println("Original count: " + Offer_DF.count())
          Offer_DF.printSchema()

          println(offer_join5.toDF().count())
          offer_join5.toDF().printSchema()
          ...
          println("Original count: " + Review_DF.count())
          Review_DF.printSchema()

          println(review_join4.toDF().count())
          review_join4.toDF().printSchema()
