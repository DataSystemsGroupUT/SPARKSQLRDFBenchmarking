...
object WidePropertyTableQP {
  def main(args: Array[String]): Unit = {
    ...
    println("Starting Querying WPT Partitioned Parquet!")
    ...
    val spark = SparkSession
      .builder()
      .appName("RDFBench Parquet WPT Partitioned")
      .getOrCreate()
    ...
    val WPT_DF = spark.read.format("parquet").load(s"$path/$partitionType/Parquet/WPT$ds.parquet").toDF()

    WPT_DF.createOrReplaceTempView("WPT")

    //create file to write the query run time results
    val fos = new FileOutputStream(new File(s"/home/hadoop/RDFBenchMarking/logs/$ds/parquet/WPT/$partitionType.txt"), true)
