...
object VerticalTablesPartition {
  def main(args: Array[String]): Unit = {
    ...
    val vpText = spark.read.format("parquet").load(path + "VHDFS/Parquet/" + "sorg_text.parquet")
    ...
    val vpdescription = spark.read.format("parquet").load(path + "VHDFS/Parquet/" + "sorg_description.parquet")
    ...
    if (partitionType == "subject") {
      ...
      vpText.repartition($"Subject").write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(path + "Subject/Parquet/" + "sorg_text.parquet")
      ...
      vpdescription.repartition($"Subject").write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(path + "Subject/Parquet/" + "sorg_description.parquet")
      ...
      vpText.repartition(84).write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(path + "Horizontal/Parquet/" + "sorg_text.parquet")
      ...
      vpdescription.repartition(84).write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(path + "Horizontal/Parquet/" + "sorg_description.parquet")
