...
object SingleStatementTablePartition {
  def main(args: Array[String]): Unit = {
    ...
    {
     RDFDF.repartition(84, $"Subject").write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(s"$path/SingleStmtTableSubject.parquet")
     println("Parquet ST partitioned and saved! Subject based Partitioning!")
    }

    else if (partitionType == "predicate")
    {
      RDFDF.repartition(84, $"Predicate").write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(s"$path/SingleStmtTablePredicate.parquet")
      println("Parquet ST partitioned and saved! Predicate based Partitioning!")
    }

    else if (partitionType == "horizontal")
    {
      RDFDF.repartition(84).write.option("header", "true").format("parquet").mode(SaveMode.Overwrite).save(s"$path/SingleStmtTableHorizontal.parquet")
