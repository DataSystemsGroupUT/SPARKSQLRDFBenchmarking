...
object VerticalTables {
  def main(args: Array[String]): Unit = {
    ...
    val vpText = spark.read.format("orc").load(path+ "ORC/" + "sorg_text.orc")
    ...
    val vpTitle = spark.read.format("orc").load(path+ "ORC/" + "og_title.orc")
    val vpRevTitle = spark.read.format("orc").load(path+ "ORC/" + "rev_title.orc")
    ...
    val vpdescription = spark.read.format("orc").load(path+ "ORC/" + "sorg_description.orc")
    ...
    vpTitle.createOrReplaceTempView("og_title")
