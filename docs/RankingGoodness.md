...
For instance, let's consider the R$_s$ rank and the 100M dataset evaluation. The top-3 ranked configurations are $\mathcal{R}_s^3$=\{b.iii.2,b.iii.1\textbf{,b.iii.4}\} (check \ref{fig:experimentsarch}), that overlaps only with the bottom-3 ranked configurations query Q4. i.e., $\mathcal{Q}4_{3}$=\{b.iii.3,\textbf{b.iii.4},a.iii.2\}.
